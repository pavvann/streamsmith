...
3. Vaultpilot: set the cloud source to the hosted pipeline persistently in `apps/vaultpilot/.env` (`CH_CLOUD_DATABASE=vaultflows_hosted` and the manifest/receipt paths the app reads; find the exact variable names in `apps/vaultpilot/src/env.ts`). Run `pnpm --filter @streamsmith/vaultpilot agent:once --source cloud` (dry run, never `--execute`). The snapshot must show the hosted mode and receipt hashes and a real decision (the wallet holds no position, so `hold` with that reason is expected; a staleness refusal is not).
