...
- **Hooks & Policies**: `HcsAuditTrailHook(tools[], topicId)` writes every tool execution to an HCS topic (warns about HIP-991 paid topics); `HolAuditTrailHook`; `MaxRecipientsPolicy`, `RejectToolPolicy`. Custom `AbstractPolicy` can block execution — this is how you build agent spending caps.
