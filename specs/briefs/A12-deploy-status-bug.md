# Brief A12 — Fix `streamsmith deploy status` failing with "Unexpected end of JSON input" against ClickHouse Cloud.

This repository, branch main. Do NOT commit; do NOT git checkout/switch/stash; pnpm only; you own packages/streamsmith/** only. Keep `pnpm typecheck && pnpm test` green (88 tests now).
