# TASKS — master tracker

Legend: `[ ]` todo · `[~]` in progress · `[x]` done · `[!]` blocked · **M**ust / **S**hould / **C**ould · owner: **OP**=operator (accounts, funds, recording), **PRJ**=project-level, **A1..An**=workstreams. Deadline **Sept 13 12:00 EDT (21:30 IST)**. Freeze **Sept 11 23:59 IST**. Kill criterion **Sept 10 12:00 IST**: one custom-module row Graph Market → sink.

## 0. Accounts, access, money (operator; unblockers)
- [x] T0.1 M PRJ · Public repo github.com/pavvann/streamsmith created and pushed Sept 9.
- [x] T0.2 M OP · (done Sept 10 13:08 IST; token in .env, .substreams.env ignored) `substreams auth` device login (Graph Market streaming key) → `SUBSTREAMS_API_TOKEN` in `.env`. Accept: `substreams run` returns blocks. dep: T1.1
- [x] T0.3 M OP · (approved 21:55 IST; token in .portal-token.json, org 0vaci288…) Graph Market Portal device-code login for hosted sink (via `thegraph-market-api` skill flow). Accept: `ListDeployments` succeeds. dep: T1.1
- [x] T0.4 M OP+PRJ · (done 20:05 IST: ap-south-1 service; db vaultflows; users sink (DDL/insert/views verified) and ro (read-only verified, writes refused, system.tables/columns readable); creds in .env as CH_CLOUD_*) ClickHouse Cloud
- [~] T0.5 M OP · (both Earn vaults active per dashboard, wrappers recorded; gas is paid from the wallet's own ETH balance; sponsorship is not used) Privy dashboard: (1) app → App ID + secret; (2) Wallets › Authorization keys › two keys (treasurer, agent), save private keys; (3) Wallet infrastructure › Earn › deploy fee wrapper for Gauntlet USDC Prime and Steakhouse Prime Instant → two `vault_id`s; (4) Gas sponsorship › App pays › Base. Fill apps/vaultpilot/.env. Accept: `pnpm --filter @streamsmith/vaultpilot spike` passes.
- [ ] T0.6 S OP · Privy: attempt Organization creation; report gated or not. Fallback: plain business-owned wallet.
- [ ] T0.7 M OP · Fund the Privy business wallet `0xcdC8B69799bCb135C04A1052b918787125571fDC` with ~50 USDC on Base plus ~0.002 ETH for gas (transactions are not sponsored). Accept: balance on Basescan.
- [ ] T0.8 M OP · Microphone + 1080p screen capture ready for Sept 12.
- [x] T0.9 S OP · Solo team (confirmed 20:20 IST).

## 1. Toolchain and spikes (Sept 9)
- [x] T0.11 M PRJ · (resolved Sept 10 13:20 IST; 1.5 GB → 12 GB free) **Build capacity on the host.** ~1.6 GB free is not enough for a Rust workspace release build (lto) or the ClickHouse image; local builds and Docker are blocked until ≥15 GB is available. Plan B: GitHub Codespace (needs `gh auth refresh -s codespace`).
...
- [x] T1.4 M PRJ · (observation at 51093000 and primary range both live and matching the on-chain reference exactly; evidence in runs/live/) `substreams run` Pinax erc4626 on Base for a 200-block historical range containing pinned-vault activity. Accept: real Deposit/Withdraw rows. dep: T0.2, T2.2
...
- [x] T1.6 M A2 · (done Sept 12: deployment `depnywi036749442f3c55e7` → database `vaultflows_hosted`; rows grow, head advances from start block 51001200; receipt receipts/erc4626-flows-v0.1.0-20260912T103328Z-vipc.json records `deploymentMode: graph-market-hosted`; the first attempt's start-command failure and its fix are in docs/build/sink-spike.md §7.2) Hosted sink spike on Graph Market with the published package → ClickHouse Cloud. Accept: rows grow, head advances. dep: T0.3, T0.4
...
- [x] T2.6 M PRJ · (tagged contract-v1 at commit f27d550, Sept 11 07:40 IST, after blind rehearsal passed gate 18/18) Freeze: tag `contract-v1` after review. Start block is 51001200 (quota decision 2026-09-10). NOTE: `buf.build/streamingfast/substreams-sink-sql` is retired; use `buf.build/streamingfast/substreams` (A5 manifest must change).
...
- [x] T6.4 M A10b+A17 · (Next 15; redesigned to the palette in the design spec: Fraunces/Source Serif/Source Sans/JetBrains Mono, decision hero, sparklines, meters; docs/build/ui-redesign.png) One-screen UI (Next.js): position, two observations + exact window, decision + rule, policy result, provenance, tx links, control/revocation info.
- [~] T6.5 M A10b+PRJ · (demo:denied verified live; deposit/rotate need funding) Demo: denied unapproved vault, then approved below-cap action from live signal. Initial funding into the lower-growth vault.
...
- [ ] T7.7 M OP+PRJ · Clean-room verification (fresh container): links, package import, MCP install, deployment status, video playback.
- [ ] T7.8 M OP · Submission form: partners = The Graph (both tracks) + Privy (both tracks); Start Fresh pool; repo; video; per-partner "how we used it" + feedback text. Submit before 12:00 EDT Sept 13. No force pushes after.
...
- [x] T8.0 M PRJ · **Standing rule: no workstream starts without a written brief committed under `specs/briefs/`.**
- [ ] T8.1 M PRJ · Commit every logical unit with meaningful messages; never a single dump.
- [ ] T8.2 M PRJ · Secrets only in `.env`; `.env.example` in every package; secret scan before each push.
- [ ] T8.3 M PRJ · Licenses: MIT for our code; respect Pinax/StreamingFast licenses on imports.
- [ ] T8.4 S PRJ · Friction logged to `feedback/*.md` as it happens.

## Won't
Bazantic, ERC-8004 module, Chainlink, Arc, Hedera, quorum ceremony, token, landing page.
- [x] T0.10 M OP · (done Sept 10 ~20:45 IST) Register the project + team on the ETHGlobal ETHOnline 2026 dashboard; Start Fresh pool.
