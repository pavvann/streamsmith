...
| World | Selfie Check = World ID 3.0 `selfieCheckLegacy` preset, **medium assurance** (liveness + face continuity, not one-person-one-account), **beta, access by email**. Sandbox App = TestFlight/Play invite. AgentBook on World Chain, Base, Base Sepolia. AgentKit track is Continuity-only. | Request access today: the World developer-access address (on the Selfie Check docs page), Developer Portal Sandbox tab, the World ID Sandbox access address (Developer Portal), prize form. |
...
3. **Is there a pre-existing project to enter as Continuity?** $20.2K is Continuity-only in a brand-new track with likely thin competition (World AgentKit $3.5K, Graph AI $5K, Arc $3.2K, Ledger $1.5K…). A project is either Start Fresh or Continuity, never both, so this would be a separate submission if rules allow more than one per team (verify).
...
- [ ] World: email the World developer-access address (on the Selfie Check docs page) for Selfie Check beta; Developer Portal → World ID Sandbox tab; the World ID Sandbox access address (Developer Portal); prize form `https://forms.gle/mqbaiwMvX5MzmKdY8`
