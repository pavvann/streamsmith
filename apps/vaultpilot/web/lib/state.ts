  ...
  if (existsSync(SNAPSHOT_FILE)) {
    try {
      return {
        ...
        originDetail: `${SNAPSHOT_FILE} (written ${statSync(SNAPSHOT_FILE).mtime.toISOString()})`,
        readAt,
        error: null,
      };
    } catch (e) {
      return {snapshot: null, origin: 'unavailable', originDetail: SNAPSHOT_FILE, readAt, error: (e as Error).message};
    }
  }
  return {
    snapshot: null,
    origin: 'unavailable',
    originDetail: SNAPSHOT_FILE,
    readAt,
    error:
